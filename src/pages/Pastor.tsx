...
export default function Pastor({ settings }: PastorProps) {
  return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-xl font-serif text-slate-400">{settings.pastorName || 'Rev. Juan Carlos Pérez'}</p>
          </div>
          ...
          <div className="relative p-8 bg-white rounded-[2.5rem] border border-slate-50 shadow-soft italic text-lg text-slate-600 leading-relaxed group">
            ...
            <p className="relative z-10 whitespace-pre-wrap">
              "{settings.pastorQuote || "Nuestra verdadera pasión es ver vidas transformadas radicalmente por el poder restaurador del Evangelio. En Emanuel no solo encontrarás una congregación, sino un hogar donde crecemos juntos en el amor de Cristo."}"
            </p>
          </div>

          <div className="space-y-6 text-slate-500 text-base leading-relaxed font-medium whitespace-pre-wrap">
            {settings.pastorBio ? (
              settings.pastorBio.split('\n\n').map((para, i) => (
                <p key={i}>{para}</p>
              ))
            ) : (
              <>
