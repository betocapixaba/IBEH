...
import { ChurchSettings, Service, Event, GalleryItem, AdminAccount } from '../types';
import { 
  Plus, 
  Trash2, 
  Save, 
...
export default function AdminDashboard({ settings, services, events, gallery, onRefresh }: AdminDashboardProps) {
  ...
  const [activePanel, setActivePanel] = useState<'settings' | 'services' | 'events' | 'gallery' | 'users'>('settings');
    ...
    return (
      <div className="pt-36 pb-24 max-w-lg mx-auto px-4">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-8 md:p-10 space-y-8 relative overflow-hidden">
          ...
          {/* Quick Info / Fast Credential Panel for Easy First Login */}
          <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 text-amber-800 text-xs space-y-1">
            <p className="font-bold flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5" /> Primer Acceso / Demo:
            </p>
            <p>Utilice la credencial maestra por defecto:</p>
            <div className="flex gap-4 mt-1 bg-white/50 p-2 rounded-lg border border-amber-200/50 font-mono">
              <p><strong>ID:</strong> admin</p>
              <p><strong>Password:</strong> admin123</p>
            </div>
            <p className="text-[10px] text-amber-700/80 pt-1">El master admin puede revocar accesos y crear cuentas adicionales de forma inmediata.</p>
          </div>
        </div>
      </div>
    );
